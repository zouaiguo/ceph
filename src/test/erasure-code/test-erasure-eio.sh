...
function delete_pool() {
    ...
    $CEPH_BIN/ceph osd erasure-code-profile rm myprofile
}
...
function rados_get() {
    ...
    if [ $expect = "1" ];
    then
        ! $CEPH_BIN/rados --pool $poolname get $objname $dir/COPY
        return
    fi
    #
...
function inject_eio() {
    ...
    CEPH_ARGS='' $CEPH_BIN/ceph --admin-daemon $dir/ceph-osd.$osd_id.asok \
