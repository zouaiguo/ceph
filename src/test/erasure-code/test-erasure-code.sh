...
function run() {
    ...
    # check that erasure code plugins are preloaded
    CEPH_ARGS='' $CEPH_BIN/ceph --admin-daemon $dir/ceph-osd.0.asok log flush || return 1
...
function delete_pool() {
    ...
    $CEPH_BIN/ceph osd pool delete $poolname $poolname --yes-i-really-really-mean-it
}
...
function rados_put_get() {
    ...
    #
    $CEPH_BIN/rados --pool $poolname put $objname $dir/ORIGINAL || return 1
    $CEPH_BIN/rados --pool $poolname get $objname $dir/COPY || return 1
    ...
    $CEPH_BIN/ceph osd out ${initial_osds[$last]} || return 1
    ! get_osds $poolname $objname | grep '\<'${initial_osds[$last]}'\>' || return 1
    $CEPH_BIN/rados --pool $poolname get $objname $dir/COPY || return 1
    diff $dir/ORIGINAL $dir/COPY || return 1
    $CEPH_BIN/ceph osd in ${initial_osds[$last]} || return 1
...
function rados_osds_out_in() {
    ...
    for osd in 0 1 ; do
      $CEPH_BIN/ceph osd out ${osds[$osd]} || return 1
    done
    wait_for_clean || return 1
    #
    ...
    #
    for osd in 0 1 ; do
      $CEPH_BIN/ceph osd in ${osds[$osd]} || return 1
    done
    wait_for_clean || return 1
...
function TEST_rados_put_get_lrc_advanced() {
    ...
    $CEPH_BIN/ceph osd erasure-code-profile set $profile \
        plugin=lrc \
        mapping=DD_ \
        ruleset-steps='[ [ "chooseleaf", "osd", 0 ] ]' \
        layers='[ [ "DDc", "" ] ]'  || return 1
    $CEPH_BIN/ceph osd pool create $poolname 12 12 erasure $profile \
        || return 1

    rados_put_get $dir $poolname || return 1

    delete_pool $poolname
    $CEPH_BIN/ceph osd erasure-code-profile rm $profile
}
...
function TEST_rados_put_get_isa() {
    ...
    $CEPH_BIN/ceph osd erasure-code-profile set profile-isa \
        plugin=isa \
        ruleset-failure-domain=osd || return 1
    $CEPH_BIN/ceph osd pool create $poolname 1 1 erasure profile-isa \
        || return 1
...
function TEST_alignment_constraints() {
    ...
    #
    local stripe_width=$($CEPH_BIN/ceph-conf --show-config-value osd_pool_erasure_code_stripe_width)
    local block_size=$((stripe_width - 1))
    dd if=/dev/zero of=$dir/ORIGINAL bs=$block_size count=2
    $CEPH_BIN/rados --block-size=$block_size \
...
function chunk_size() {
    local stripe_width=$($CEPH_BIN/ceph-conf --show-config-value osd_pool_erasure_code_stripe_width)
    eval local $($CEPH_BIN/ceph osd erasure-code-profile get default | grep k=)
    echo $(($stripe_width / $k))
}
...
function verify_chunk_mapping() {
    ...
    $CEPH_BIN/rados --pool $poolname put SOMETHING$poolname $dir/ORIGINAL || return 1
    $CEPH_BIN/rados --pool $poolname get SOMETHING$poolname $dir/COPY || return 1
    local -a osds=($(get_osds $poolname SOMETHING$poolname))
    for (( i = 0; i < ${#osds[@]}; i++ )) ; do
        $CEPH_BIN/ceph daemon osd.${osds[$i]} flush_journal
    done
    diff $dir/ORIGINAL $dir/COPY || return 1
    rm $dir/COPY
...
function TEST_chunk_mapping() {
    ...
    $CEPH_BIN/ceph osd erasure-code-profile set remap-profile \
        plugin=lrc \
        layers='[ [ "_DD", "" ] ]' \
        mapping='_DD' \
        ruleset-steps='[ [ "choose", "osd", 0 ] ]' || return 1
    $CEPH_BIN/ceph osd erasure-code-profile get remap-profile
    $CEPH_BIN/ceph osd pool create remap-pool 12 12 erasure remap-profile \
        || return 1
    ...
    $CEPH_BIN/ceph osd erasure-code-profile rm remap-profile
}
