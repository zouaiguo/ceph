...
function TEST_set() {
    ...
    #
    $CEPH_BIN/ceph osd erasure-code-profile set $profile 2>&1 || return 1
    $CEPH_BIN/ceph osd erasure-code-profile get $profile | \
    ...
    #
    ! $CEPH_BIN/ceph osd erasure-code-profile set $profile > $dir/out 2>&1 || return 1
    grep 'will not override' $dir/out || return 1
    $CEPH_BIN/ceph osd erasure-code-profile set $profile key=other --force || return 1
    $CEPH_BIN/ceph osd erasure-code-profile get $profile | \
        grep key=other || return 1

    $CEPH_BIN/ceph osd erasure-code-profile rm $profile # cleanup
}
...
function TEST_ls() {
    ...
    local profile=myprofile
    ! $CEPH_BIN/ceph osd erasure-code-profile ls | grep $profile || return 1
    $CEPH_BIN/ceph osd erasure-code-profile set $profile 2>&1 || return 1
    $CEPH_BIN/ceph osd erasure-code-profile ls | grep $profile || return 1
    $CEPH_BIN/ceph --format xml osd erasure-code-profile ls | \
...
function TEST_rm() {
    ...
    $CEPH_BIN/ceph osd erasure-code-profile set $profile 2>&1 || return 1
    $CEPH_BIN/ceph osd erasure-code-profile ls | grep $profile || return 1
    $CEPH_BIN/ceph osd erasure-code-profile rm $profile || return 1
    ! $CEPH_BIN/ceph osd erasure-code-profile ls | grep $profile || return 1
    $CEPH_BIN/ceph osd erasure-code-profile rm WRONG 2>&1 | \
        grep "WRONG does not exist" || return 1

    $CEPH_BIN/ceph osd erasure-code-profile set $profile || return 1
    $CEPH_BIN/ceph osd pool create poolname 12 12 erasure $profile || return 1
    ! $CEPH_BIN/ceph osd erasure-code-profile rm $profile > $dir/out 2>&1 || return 1
    grep "poolname.*using.*$profile" $dir/out || return 1
    $CEPH_BIN/ceph osd pool delete poolname poolname --yes-i-really-really-mean-it || return 1
    $CEPH_BIN/ceph osd erasure-code-profile rm $profile || return 1
...
function TEST_get() {
    ...
    $CEPH_BIN/ceph osd erasure-code-profile get $default_profile | \
        grep plugin=jerasure || return 1
    $CEPH_BIN/ceph --format xml osd erasure-code-profile get $default_profile | \
        grep '<plugin>jerasure</plugin>' || return 1
    ! $CEPH_BIN/ceph osd erasure-code-profile get WRONG > $dir/out 2>&1 || return 1
...
function TEST_set_idempotent() {
    ...
    #
    $CEPH_BIN/ceph osd erasure-code-profile set default k=2 m=1 2>&1 || return 1
    local profile
    #
    ...
    ! $CEPH_BIN/ceph osd erasure-code-profile ls | grep $profile || return 1
    $CEPH_BIN/ceph osd erasure-code-profile set $profile k=2 ruleset-failure-domain=osd 2>&1 || return 1
    $CEPH_BIN/ceph osd erasure-code-profile ls | grep $profile || return 1
    $CEPH_BIN/ceph osd erasure-code-profile set $profile k=2 ruleset-failure-domain=osd 2>&1 || return 1
    $CEPH_BIN/ceph osd erasure-code-profile rm $profile # cleanup
    ...
    ! $CEPH_BIN/ceph osd erasure-code-profile ls | grep $profile || return 1
    $CEPH_BIN/ceph osd erasure-code-profile set $profile plugin=lrc k=4 m=2 l=3 ruleset-failure-domain=osd 2>&1 || return 1
    $CEPH_BIN/ceph osd erasure-code-profile ls | grep $profile || return 1
    $CEPH_BIN/ceph osd erasure-code-profile set $profile plugin=lrc k=4 m=2 l=3 ruleset-failure-domain=osd 2>&1 || return 1
    $CEPH_BIN/ceph osd erasure-code-profile rm $profile # cleanup
}
...
function TEST_format_json() {
    ...
    $CEPH_BIN/ceph --format json osd erasure-code-profile get default | \
