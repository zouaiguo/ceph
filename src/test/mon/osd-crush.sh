...
function TEST_crush_rule_create_simple() {
    ...
    $CEPH_BIN/ceph --format xml osd crush rule dump replicated_ruleset | \
    ...
    $CEPH_BIN/ceph osd crush rule create-simple $ruleset $root $failure_domain 2>&1 | \
        grep "$ruleset already exists" || return 1
    $CEPH_BIN/ceph --format xml osd crush rule dump $ruleset | \
        egrep '<op>take</op><item>[^<]+</item><item_name>'$root'</item_name>' | \
        grep '<op>choose_firstn</op><num>0</num><type>'$failure_domain'</type>' || return 1
    $CEPH_BIN/ceph osd crush rule rm $ruleset || return 1
}
...
function TEST_crush_rule_dump() {
    ...
    $CEPH_BIN/ceph osd crush rule create-erasure $ruleset || return 1
    local expected
    expected="<rule_name>$ruleset</rule_name>"
    $CEPH_BIN/ceph --format xml osd crush rule dump $ruleset | grep $expected || return 1
    expected='"rule_name": "'$ruleset'"'
    $CEPH_BIN/ceph osd crush rule dump | grep "$expected" || return 1
    ! $CEPH_BIN/ceph osd crush rule dump non_existent_ruleset || return 1
    $CEPH_BIN/ceph osd crush rule rm $ruleset || return 1
}
...
function TEST_crush_rule_rm() {
    ...
    $CEPH_BIN/ceph osd crush rule create-erasure $ruleset default || return 1
    $CEPH_BIN/ceph osd crush rule ls | grep $ruleset || return 1
    $CEPH_BIN/ceph osd crush rule rm $ruleset || return 1
    ! $CEPH_BIN/ceph osd crush rule ls | grep $ruleset || return 1
}
...
function TEST_crush_rule_create_erasure() {
    ...
    #
    $CEPH_BIN/ceph osd crush rule create-erasure $ruleset || return 1
    $CEPH_BIN/ceph osd crush rule create-erasure $ruleset 2>&1 | \
        grep "$ruleset already exists" || return 1
    $CEPH_BIN/ceph --format xml osd crush rule dump $ruleset | \
        egrep '<op>take</op><item>[^<]+</item><item_name>default</item_name>' | \
    ...
    $CEPH_BIN/ceph tell mon.\* injectargs --crushtool "false"

    expect_failure $dir "Error EINVAL" \
        $CEPH_BIN/ceph osd pool create mypool 1 1 erasure || return 1
}

function check_ruleset_id_match_rule_id() {
    local rule_name=$1
    rule_id=`$CEPH_BIN/ceph osd crush rule dump $rule_name | grep "\"rule_id\":" | awk -F ":|," '{print int($2)}'`
    ruleset_id=`$CEPH_BIN/ceph osd crush rule dump $rule_name | grep "\"ruleset\":"| awk -F ":|," '{print int($2)}'`
...
function generate_manipulated_rules() {
    local dir=$1
    $CEPH_BIN/ceph osd crush add-bucket $root host
    $CEPH_BIN/ceph osd crush rule create-simple test_rule1 $root osd firstn || return 1
    $CEPH_BIN/ceph osd crush rule create-simple test_rule2 $root osd firstn || return 1
    $CEPH_BIN/ceph osd getcrushmap -o $dir/original_map
    $CEPH_BIN/crushtool -d $dir/original_map -o $dir/decoded_original_map
    ...
    $CEPH_BIN/crushtool -c $dir/decoded_original_map -o $dir/new_map
    $CEPH_BIN/ceph osd setcrushmap -i $dir/new_map

    $CEPH_BIN/ceph osd crush rule dump
}
...
function TEST_crush_ruleset_match_rule_when_creating() {
    ...
    $CEPH_BIN/ceph osd crush rule create-simple special_rule_simple $root osd firstn || return 1
...
function TEST_add_ruleset_failed() {
    ...
    $CEPH_BIN/ceph osd crush rule create-simple test_rule1 $root osd firstn || return 1
    $CEPH_BIN/ceph osd crush rule create-simple test_rule2 $root osd firstn || return 1
    $CEPH_BIN/ceph osd getcrushmap > $dir/crushmap || return 1
    $CEPH_BIN/crushtool --decompile $dir/crushmap > $dir/crushmap.txt || return 1
    for i in $(seq 3 255)
        do
            cat <<EOF
rule test_rule$i {
...
EOF
    ...
    $CEPH_BIN/crushtool --compile $dir/crushmap.txt -o $dir/crushmap || return 1
    $CEPH_BIN/ceph osd setcrushmap -i $dir/crushmap  || return 1
    $CEPH_BIN/ceph osd crush rule create-simple test_rule_nospace $root osd firstn 2>&1 | grep "Error ENOSPC" || return 1
...
function TEST_crush_rename_bucket() {
    ...
    $CEPH_BIN/ceph osd crush add-bucket host1 host
    ! $CEPH_BIN/ceph osd tree | grep host2 || return 1
    $CEPH_BIN/ceph osd crush rename-bucket host1 host2 || return 1
    $CEPH_BIN/ceph osd tree | grep host2 || return 1
    $CEPH_BIN/ceph osd crush rename-bucket host1 host2 || return 1 # idempotency
    $CEPH_BIN/ceph osd crush rename-bucket nonexistent something 2>&1 | grep "Error ENOENT" || return 1
}
...
function TEST_crush_reject_empty() {
    ...
    :> $empty_map.txt
    $CEPH_BIN/crushtool -c $empty_map.txt -o $empty_map.map || return 1
    expect_failure $dir "Error EINVAL" \
        $CEPH_BIN/ceph osd setcrushmap -i $empty_map.map || return 1
}
...
function TEST_crush_tree() {
    ...
    $CEPH_BIN/ceph osd crush tree --format=xml | \
...
function TEST_crush_repair_faulty_crushmap() {
    ...
    local crushtool_path_old=`$CEPH_BIN/ceph-conf --show-config-value crushtool`
    $CEPH_BIN/ceph tell mon.\* injectargs --crushtool "true"

    $CEPH_BIN/ceph osd setcrushmap -i $empty_map.map || return 1
    # should be an empty crush map without any buckets
    ! test $($CEPH_BIN/ceph osd crush dump --format=xml | \
