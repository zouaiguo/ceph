...
function TEST_default_deprectated_0() {
    ...
    $CEPH_BIN/ceph osd pool get rbd crush_ruleset | grep 'ruleset: '$expected || return 1
    $CEPH_BIN/ceph osd crush rule dump replicated_ruleset | grep '"ruleset": '$expected || return 1
    CEPH_ARGS='' $CEPH_BIN/ceph --admin-daemon $dir/ceph-mon.a.asok log flush || return 1
...
function TEST_default_deprectated_2() {
    ...
    $CEPH_BIN/ceph osd pool get rbd crush_ruleset | grep 'ruleset: '$expected || return 1
    ! $CEPH_BIN/ceph --format json osd dump | grep '"crush_ruleset":'$unexpected || return 1
    $CEPH_BIN/ceph osd crush rule dump replicated_ruleset | grep '"ruleset": '$expected || return 1
    CEPH_ARGS='' $CEPH_BIN/ceph --admin-daemon $dir/ceph-mon.a.asok log flush || return 1
    grep "osd_pool_default_crush_rule is deprecated " $dir/mon.a.log || return 1
}
...
function TEST_erasure_invalid_profile() {
    ...
    ! $CEPH_BIN/ceph osd pool create $poolname 12 12 erasure $notaprofile || return 1
    ! $CEPH_BIN/ceph osd erasure-code-profile ls | grep $notaprofile || return 1
}
...
function TEST_erasure_crush_rule() {
    ...
    ! $CEPH_BIN/ceph osd crush rule ls | grep $crush_ruleset || return 1
    $CEPH_BIN/ceph osd crush rule create-erasure $crush_ruleset
    $CEPH_BIN/ceph osd crush rule ls | grep $crush_ruleset
    local poolname
    poolname=pool_erasure1
    ! $CEPH_BIN/ceph --format json osd dump | grep '"crush_ruleset":1' || return 1
    $CEPH_BIN/ceph osd pool create $poolname 12 12 erasure default $crush_ruleset
    $CEPH_BIN/ceph --format json osd dump | grep '"crush_ruleset":1' || return 1
    #
    # a crush ruleset by the name of the pool is implicitly created
    #
    poolname=pool_erasure2
    $CEPH_BIN/ceph osd erasure-code-profile set myprofile
    $CEPH_BIN/ceph osd pool create $poolname 12 12 erasure myprofile
    $CEPH_BIN/ceph osd crush rule ls | grep $poolname || return 1
    #
    # a non existent crush ruleset given in argument is an error
    # http://tracker.ceph.com/issues/9304
    #
    poolname=pool_erasure3
    ! $CEPH_BIN/ceph osd pool create $poolname 12 12 erasure myprofile INVALIDRULESET || return 1
}

function TEST_erasure_code_profile_default() {
    local dir=$1
    run_mon $dir a || return 1
    $CEPH_BIN/ceph osd erasure-code-profile rm default || return 1
    ! $CEPH_BIN/ceph osd erasure-code-profile ls | grep default || return 1
    $CEPH_BIN/ceph osd pool create $poolname 12 12 erasure default
    $CEPH_BIN/ceph osd erasure-code-profile ls | grep default || return 1
}
...
function TEST_erasure_crush_stripe_width() {
    ...
    stripe_width=$($CEPH_BIN/ceph-conf --show-config-value osd_pool_erasure_code_stripe_width)
    $CEPH_BIN/ceph osd pool create pool_erasure 12 12 erasure
    $CEPH_BIN/ceph --format json osd dump | tee $dir/osd.json
...
function TEST_erasure_crush_stripe_width_padded() {
    ...
    $CEPH_BIN/ceph osd pool create pool_erasure 12 12 erasure
    $CEPH_BIN/ceph osd dump | tee $dir/osd.json
...
function TEST_erasure_code_pool() {
    local dir=$1
    run_mon $dir a || return 1
    $CEPH_BIN/ceph --format json osd dump > $dir/osd.json
    local expected='"erasure_code_profile":"default"'
    ! grep "$expected" $dir/osd.json || return 1
    $CEPH_BIN/ceph osd pool create erasurecodes 12 12 erasure
    $CEPH_BIN/ceph --format json osd dump | tee $dir/osd.json
    grep "$expected" $dir/osd.json > /dev/null || return 1

    $CEPH_BIN/ceph osd pool create erasurecodes 12 12 erasure 2>&1 | \
        grep 'already exists' || return 1
    $CEPH_BIN/ceph osd pool create erasurecodes 12 12 2>&1 | \
...
function TEST_replicated_pool_with_ruleset() {
    ...
    local root=host1
    $CEPH_BIN/ceph osd crush add-bucket $root host
    local failure_domain=osd
    local poolname=mypool
    $CEPH_BIN/ceph osd crush rule create-simple $ruleset $root $failure_domain || return 1
    $CEPH_BIN/ceph osd crush rule ls | grep $ruleset
    $CEPH_BIN/ceph osd pool create $poolname 12 12 replicated $ruleset 2>&1 | \
        grep "pool 'mypool' created" || return 1
    rule_id=`$CEPH_BIN/ceph osd crush rule dump $ruleset | grep "rule_id" | awk -F[' ':,] '{print $4}'`
    $CEPH_BIN/ceph osd pool get $poolname crush_ruleset  2>&1 | \
        grep "crush_ruleset: $rule_id" || return 1
    #non-existent crush ruleset
    $CEPH_BIN/ceph osd pool create newpool 12 12 replicated non-existent 2>&1 | \
...
function TEST_replicated_pool_with_non_existent_default_ruleset_0() {
    ...
    $CEPH_BIN/ceph tell mon.a injectargs -- \
        --osd_pool_default_crush_replicated_ruleset 66 || return 1
    $CEPH_BIN/ceph osd pool create mypool 12 12 replicated 2>&1 | \
...
function TEST_erasure_code_pool_lrc() {
    ...
    $CEPH_BIN/ceph osd erasure-code-profile set LRCprofile \
             plugin=lrc \
             mapping=DD_ \
             layers='[ [ "DDc", "" ] ]' || return 1
    ...
    ! grep "$expected" $dir/osd.json || return 1
    $CEPH_BIN/ceph osd pool create $poolname 12 12 erasure LRCprofile
    $CEPH_BIN/ceph --format json osd dump | tee $dir/osd.json
    grep "$expected" $dir/osd.json > /dev/null || return 1
    $CEPH_BIN/ceph osd crush rule ls | grep $poolname || return 1
}
...
function TEST_replicated_pool() {
    local dir=$1
    run_mon $dir a || return 1
    $CEPH_BIN/ceph osd pool create replicated 12 12 replicated replicated_ruleset 2>&1 | \
        grep "pool 'replicated' created" || return 1
    $CEPH_BIN/ceph osd pool create replicated 12 12 replicated replicated_ruleset 2>&1 | \
        grep 'already exists' || return 1
    # default is replicated
    $CEPH_BIN/ceph osd pool create replicated1 12 12 2>&1 | \
        grep "pool 'replicated1' created" || return 1
    # default is replicated, pgp_num = pg_num
    $CEPH_BIN/ceph osd pool create replicated2 12 2>&1 | \
        grep "pool 'replicated2' created" || return 1
    $CEPH_BIN/ceph osd pool create replicated 12 12 erasure 2>&1 | \
...
function TEST_no_pool_delete() {
    local dir=$1
    run_mon $dir a || return 1
    $CEPH_BIN/ceph osd pool create foo 1 || return 1
    $CEPH_BIN/ceph tell mon.a injectargs -- --no-mon-allow-pool-delete || return 1
    ! $CEPH_BIN/ceph osd pool delete foo foo --yes-i-really-really-mean-it || return 1
    $CEPH_BIN/ceph tell mon.a injectargs -- --mon-allow-pool-delete || return 1
    $CEPH_BIN/ceph osd pool delete foo foo --yes-i-really-really-mean-it || return 1
}
...
function TEST_utf8_cli() {
    ...
    $CEPH_BIN/ceph osd pool create 黄 1024 2>&1 | \
        grep "pool '黄' created" || return 1
    $CEPH_BIN/ceph osd lspools 2>&1 | \
        grep "黄" || return 1
    $CEPH_BIN/ceph -f json-pretty osd dump | \
        python -c "import json; import sys; json.load(sys.stdin)" || return 1
    $CEPH_BIN/ceph osd pool delete 黄 黄 --yes-i-really-really-mean-it
}
