...
myversion=`$CEPH_BIN/ceph-dencoder version`
...
for arversion in `ls -v $dir/archive`; do
  ...
  for type in `ls $vdir/objects`; do
    if $CEPH_BIN/ceph-dencoder type $type 2>/dev/null; then
      #echo "type $type";
      ...
      for f in `ls $vdir/objects/$type`; do
        ...
        #echo "\t$vdir/$type/$f"
        if ! $CEPH_BIN/ceph-dencoder type $type import $vdir/objects/$type/$f decode dump_json > $tmp1; then
          echo "**** failed to decode $vdir/objects/$type/$f ****"
          failed=$(($failed + 1))
          continue      
        fi
        if ! $CEPH_BIN/ceph-dencoder type $type import $vdir/objects/$type/$f decode encode decode dump_json > $tmp2; then
        ...
        # nothing.
        if ! $CEPH_BIN/ceph-dencoder type $type is_deterministic; then
