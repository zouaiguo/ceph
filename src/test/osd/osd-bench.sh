...
function TEST_bench() {
    ...
    local osd_bench_small_size_max_iops=$(CEPH_ARGS='' $CEPH_BIN/ceph-conf \
        --show-config-value osd_bench_small_size_max_iops)
    local osd_bench_large_size_max_throughput=$(CEPH_ARGS='' $CEPH_BIN/ceph-conf \
        --show-config-value osd_bench_large_size_max_throughput)
    local osd_bench_max_block_size=$(CEPH_ARGS='' $CEPH_BIN/ceph-conf \
        --show-config-value osd_bench_max_block_size)
    local osd_bench_duration=$(CEPH_ARGS='' $CEPH_BIN/ceph-conf \
    ...
    expect_failure $dir osd_bench_max_block_size \
        $CEPH_BIN/ceph tell osd.0 bench 1024 $((osd_bench_max_block_size + 1)) || return 1
    ...
    expect_failure $dir bench_small_size_max_iops \
        $CEPH_BIN/ceph tell osd.0 bench $(($max_count + 1)) $bsize || return 1
    ...
    #
    $CEPH_BIN/ceph tell osd.0 bench || return 1
}
