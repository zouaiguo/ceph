...
function TEST_config_track() {
    ...
    local osd_map_cache_size=$(CEPH_ARGS='' $CEPH_BIN/ceph-conf \
        --show-config-value osd_map_cache_size)
    local osd_map_max_advance=$(CEPH_ARGS='' $CEPH_BIN/ceph-conf \
        --show-config-value osd_map_max_advance)
    local osd_pg_epoch_persisted_max_stale=$(CEPH_ARGS='' $CEPH_BIN/ceph-conf \
    ...
    $CEPH_BIN/ceph tell osd.0 injectargs "--osd-map-cache-size $cache" || return 1
    CEPH_ARGS='' $CEPH_BIN/ceph --admin-daemon $dir/ceph-osd.0.asok log flush || return 1
    grep 'is not > osd_map_max_advance' $dir/osd.0.log || return 1
    rm $dir/osd.0.log
    CEPH_ARGS='' $CEPH_BIN/ceph --admin-daemon $dir/ceph-osd.0.asok log reopen || return 1
    ...
    $CEPH_BIN/ceph tell osd.0 injectargs "--osd-map-max-advance $advance" || return 1
    CEPH_ARGS='' $CEPH_BIN/ceph --admin-daemon $dir/ceph-osd.0.asok log flush || return 1
    grep 'is not > osd_map_max_advance' $dir/osd.0.log || return 1
    ...
    $CEPH_BIN/ceph tell osd.0 injectargs "--osd-pg-epoch-persisted-max-stale $stale" || return 1
    CEPH_ARGS='' $CEPH_BIN/ceph --admin-daemon $dir/ceph-osd.0.asok log flush || return 1
    grep 'is not > osd_pg_epoch_persisted_max_stale' $dir/osd.0.log || return 1
}
