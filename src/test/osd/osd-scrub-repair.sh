...
function add_something() {
    ...
    $CEPH_BIN/ceph osd set noscrub || return 1
    $CEPH_BIN/ceph osd set nodeep-scrub || return 1

    local payload=ABCDEF
    echo $payload > $dir/ORIGINAL
    $CEPH_BIN/rados --pool $poolname put $obj $dir/ORIGINAL || return 1
}
...
function corrupt_and_repair_two() {
    ...
    $CEPH_BIN/rados --pool $poolname get SOMETHING $dir/COPY || return 1
...
function corrupt_and_repair_erasure_coded() {
    ...
    $CEPH_BIN/ceph osd pool create $poolname 1 1 erasure $profile \
        || return 1
...
function TEST_auto_repair_erasure_coded() {
    ...
    $CEPH_BIN/ceph osd pool create $poolname 8 8 erasure myprofile || return 1

    # Put an object
    local payload=ABCDEF
    echo $payload > $dir/ORIGINAL
    $CEPH_BIN/rados --pool $poolname put SOMETHING $dir/ORIGINAL || return 1
    wait_for_clean || return 1
...
function TEST_unfound_erasure_coded() {
    ...
    $CEPH_BIN/ceph osd erasure-code-profile set myprofile \
      k=2 m=2 ruleset-failure-domain=osd || return 1
    $CEPH_BIN/ceph osd pool create $poolname 1 1 erasure myprofile \
      || return 1
    ...
    # 3) check pg state
    #
    $CEPH_BIN/ceph -s|grep "4 osds: 4 up, 4 in" || return 1
    $CEPH_BIN/ceph -s|grep "1/1 unfound" || return 1
...
function TEST_list_missing_erasure_coded() {
    ...
    $CEPH_BIN/ceph pg repair $pg
    
    for i in $(seq 0 120) ; do
        [ $i -lt 60 ] || return 1
        matches=$($CEPH_BIN/ceph pg $pg list_missing | egrep "OBJ0|OBJ1" | wc -l)
        [ $matches -eq 2 ] && break
    done
