...
CEPH_BUILD_DIR = os.environ['CEPH_BUILD_DIR']
...
def main(argv):
    ...
    CFSD_PREFIX = CEPH_BIN + "/ceph-objectstore-tool --data-path " + OSDDIR + "/{osd} --journal-path " + OSDDIR + "/{osd}.journal "
    ...
    cmd = "{path}/ceph-objectstore-tool --data-path BAD_DATA_PATH --journal-path ".format(path=CEPH_BIN) + OSDDIR + "/{osd}.journal --op list".format(osd=ONEOSD)
    ...
    cmd = "{path}/ceph-objectstore-tool --journal-path {dir}/{osd}.journal --type memstore --op list --pgid {pg}".format(dir=OSDDIR, osd=ONEOSD, pg=ONEPG, path=CEPH_BIN)
