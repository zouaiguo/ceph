...
class DeletingState {
  ...
  DeletingState(const pair<spg_t, PGRef> &in) :
    lock("DeletingState::lock"), status(QUEUED), stop_deleting(false), pgid(
	in.first), old_pg_state(in.second) {
  ...
  bool start_or_resume_clearing() {
    Mutex::Locker l(lock);
    assert(
	status == QUEUED || status == DELETED_DIR
	|| status == CLEARING_WAITING);
    if (stop_deleting) {
      status = CANCELED;
      cond.Signal();
...
class PGQueueable {
  typedef boost::variant<OpRequestRef, PGSnapTrim, PGScrub> QVariant;
  QVariant qvariant;
  int cost;
  unsigned priority;
  utime_t start_time;
  ...
  struct RunVis: public boost::static_visitor<> {
    ...
    void
      operator()(OpRequestRef &op);
    void
      operator()(PGSnapTrim &op);
    void
      operator()(PGScrub &op);
  };
  public:
  PGQueueable(OpRequestRef op) :
    qvariant(op), cost(op->get_req()->get_cost()), priority(
	op->get_req()->get_priority()), start_time(
	op->get_req()->get_recv_stamp()), owner(
...
class OSDService {
  ...
  public:
    ...
    void
      dequeue_pg(PG *pg, list<OpRequestRef> *dequeued);
	...
	return OSDMapRef();
      ...
      map<epoch_t, unsigned>::iterator i = map_reservations.insert(
	  make_pair(e, 0)).first;
      i->second++;
      return next_osdmap;
    }
    ...
    void release_map(OSDMapRef osdmap) {
      Mutex::Locker l(pre_publish_lock);
      map<epoch_t, unsigned>::iterator i = map_reservations.find(
	  osdmap->get_epoch());
  ...
  public:
    ...
    epoch_t
      note_peer_epoch(int p, epoch_t e);
    void
      forget_peer_epoch(int p, epoch_t e);

    void
      send_map(class MOSDMap *m, Connection *con);
    void
      send_incremental_map(epoch_t since, Connection *con, OSDMapRef& osdmap);
    MOSDMap *
      build_incremental_map_msg(epoch_t from, epoch_t to,
	  OSDSuperblock& superblock);bool
      should_share_map(entity_name_t name, Connection *con, epoch_t epoch,
	  OSDMapRef& osdmap, const epoch_t *sent_epoch_p);
    void
      share_map(entity_name_t name, Connection *con, epoch_t epoch,
	  OSDMapRef& osdmap, epoch_t *sent_epoch_p);
    void
      share_map_peer(int peer, Connection *con, OSDMapRef map = OSDMapRef());

    ConnectionRef
      get_con_osd_cluster(int peer, epoch_t from_epoch);
    pair<ConnectionRef, ConnectionRef>
      get_con_osd_hb(int peer, epoch_t from_epoch);  // (back, front)
    void
      send_message_osd_cluster(int peer, Message *m, epoch_t from_epoch);
    ...
    struct ScrubJob {
      ...
      explicit
	ScrubJob(const spg_t& pg, const utime_t& timestamp, bool must =
	    true);
    ...
    bool
      inc_scrubs_pending();
    void
      inc_scrubs_active(bool reserved);
    void
      dec_scrubs_pending();
    void
      dec_scrubs_active();
    ...
    void
      reply_op_error(OpRequestRef op, int err);
    void
      reply_op_error(OpRequestRef op, int err, eversion_t v, version_t uv);
    void
      handle_misdirected_op(PG *pg, OpRequestRef op);

    // -- agent shared state --
    Mutex agent_lock;
    Cond agent_cond;
    map<uint64_t, set<PGRef> > agent_queue;
    set<PGRef>::iterator agent_queue_pos;bool agent_valid_iterator;
    int agent_ops;
    set<hobject_t> agent_oids;bool agent_active;
    struct AgentThread: public Thread {
      OSDService *osd;
      AgentThread(OSDService *o) :
	osd(o) {
	}
      void *
	entry() {
    ...
    } agent_thread;bool agent_stop_flag;
    ...
    void
      queue_want_pg_temp(pg_t pgid, vector<int>& want);
    ...
    void queue_for_snap_trim(PG *pg) {
      op_wq.queue(
	  make_pair(pg,
	    PGQueueable(PGSnapTrim(pg->get_osdmap()->get_epoch()),
	      cct->_conf->osd_snap_trim_cost,
	      cct->_conf->osd_snap_trim_priority,
	      ceph_clock_now(cct), entity_inst_t())));
    }
    void queue_for_scrub(PG *pg) {
      op_wq.queue(
	  make_pair(pg,
	    PGQueueable(PGScrub(pg->get_osdmap()->get_epoch()),
    ...
    void
      pin_map_bl(epoch_t e, bufferlist &bl);
    void
      _add_map_bl(epoch_t e, bufferlist& bl);bool get_map_bl(epoch_t e,
	  bufferlist& bl) {
	Mutex::Locker l(map_cache_lock);
	return _get_map_bl(e, bl);
      }
    bool
      _get_map_bl(epoch_t e, bufferlist& bl);
    ...
    void
      pin_map_inc_bl(epoch_t e, bufferlist &bl);
    void
      _add_map_inc_bl(epoch_t e, bufferlist& bl);bool
      get_inc_map_bl(epoch_t e, bufferlist& bl);

    void
      clear_map_bl_cache_pins(epoch_t e);

    void
      need_heartbeat_peer_update();

    void
      pg_stat_queue_enqueue(PG *pg);
    void
      pg_stat_queue_dequeue(PG *pg);
    ...
    void
      _start_split(spg_t parent, const set<spg_t> &children);
    void start_split(spg_t parent, const set<spg_t> &children) {
      Mutex::Locker l(in_progress_split_lock);
      return _start_split(parent, children);
    }
    void
      mark_split_in_progress(spg_t parent, const set<spg_t> &pgs);
    void
      complete_split(const set<spg_t> &pgs);
    void
      cancel_pending_splits_for_parent(spg_t parent);
    void
      _cancel_pending_splits_for_parent(spg_t parent);bool
      splitting(spg_t pgid);
    void
      expand_pg_num(OSDMapRef old_map, OSDMapRef new_map);
    void
      _maybe_split_pgid(OSDMapRef old_map, OSDMapRef new_map, spg_t pgid);
    void
      init_splits_between(spg_t pgid, OSDMapRef frommap, OSDMapRef tomap);
    ...
    void
      update_osd_stat(vector<int>& hb_peers);
    ...
    float
      get_nearfull_ratio();
    void
      check_nearfull_warning(const osd_stat_t &stat);bool
      check_failsafe_full();bool
      too_full_for_backfill(double *ratio, double *max_ratio);
  ...
  public:
     ...
     */
    void
      retrieve_epochs(epoch_t *_boot_epoch, epoch_t *_up_epoch,
	  epoch_t *_bind_epoch) const;
    /**
     * Set the boot, up, and bind epochs. Any NULL params will not be set.
     */
    void
      set_epochs(const epoch_t *_boot_epoch, const epoch_t *_up_epoch,
...
struct C_OSD_SendMessageOnConn: public Context {
  ...
  Message *reply;
  ConnectionRef conn;
  C_OSD_SendMessageOnConn(OSDService *osd, Message *reply, ConnectionRef conn) :
    osd(osd), reply(reply), conn(conn) {
    }
  void finish(int) {
...
class OSD: public Dispatcher, public md_config_obs_t {
  ...
  public:
    ...
    virtual void
      handle_conf_change(const struct md_config_t *conf,
  ...
  protected:
    ...
    void
      _dispatch(Message *m);
    void
      dispatch_op(OpRequestRef op);bool
      dispatch_op_fast(OpRequestRef& op, OSDMapRef& osdmap);

    void
      check_osdmap_features(ObjectStore *store);

    // asok
    friend class OSDSocketHook;
    class OSDSocketHook *asok_hook;bool
      asok_command(string command, cmdmap_t& cmdmap, string format, ostream& ss);
  ...
  public:
    ...
    static hobject_t make_snapmapper_oid() {
      return hobject_t(sobject_t(object_t("snapmapper"), 0));
    }
    ...
    static void
      recursive_remove_collection(ObjectStore *store, coll_t tmp);
  ...
  private:
    ...
    void
      write_superblock();
    void
      write_superblock(ObjectStore::Transaction& t);
    int
      read_superblock();
  ...
  public:
	...
	case CEPH_MSG_OSD_OP:
	  return (static_cast<MOSDOp*>(op->get_req())->get_pg().m_seed & mask)
	    == match;
      }
      return false;
    }

    static void split_list(list<OpRequestRef> *from, list<OpRequestRef> *to,
	unsigned match, unsigned bits) {
      for (list<OpRequestRef>::iterator i = from->begin(); i != from->end();
	  ) {
    ...
    void
      update_waiting_for_pg(Session *session, OSDMapRef osdmap);
    void
      session_notify_pg_create(Session *session, OSDMapRef osdmap, spg_t pgid);
    void
      session_notify_pg_cleared(Session *session, OSDMapRef osdmap, spg_t pgid);
    void
      dispatch_session_waiting(Session *session, OSDMapRef osdmap);
    ...
    void clear_waiting_sessions() {
	  ...
	  i != session_waiting_for_pg.end(); ++i) {
	for (set<Session*>::iterator j = i->second.begin();
	  ...
	  i != session_waiting_for_map.end(); ++i) {
	(*i)->put();
      }
    ...
    void dispatch_sessions_waiting_on_map() {
	  ...
	  i != sessions_to_check.end(); sessions_to_check.erase(i++)) {
    ...
    void clear_session_waiting_on_pg(Session *session, spg_t pgid) {
      Mutex::Locker l(session_waiting_lock);
      map<spg_t, set<Session*> >::iterator i = session_waiting_for_pg.find(
	  pgid);
    ...
    void get_sessions_possibly_interested_in_pg(spg_t pgid,
	set<Session*> *sessions) {
      Mutex::Locker l(session_waiting_lock);
      while (1) {
	map<spg_t, set<Session*> >::iterator i =
	  session_waiting_for_pg.find(pgid);
	if (i != session_waiting_for_pg.end()) {
  ...
  private:
     ...
     */
    void
      osdmap_subscribe(version_t epoch, bool force_request);
    /** @} monc helpers */
	...
	bool is_unhealthy(utime_t cutoff) {
	  return !((last_rx_front > cutoff
		|| (last_rx_front == utime_t()
		  && (last_tx == utime_t() || first_tx > cutoff)))
	      && (last_rx_back > cutoff
		|| (last_rx_back == utime_t()
		  && (last_tx == utime_t()
		    || first_tx > cutoff))));
	}
...
struct HeartbeatSession: public RefCountedObject {
  int peer;
  HeartbeatSession(int p) :
    peer(p) {
    }
};
Mutex heartbeat_lock;
map<int, int> debug_heartbeat_drops_remaining;
Cond heartbeat_cond;bool heartbeat_stop;
...
void
_add_heartbeat_peer(int p);
void
_remove_heartbeat_peer(int p);bool
heartbeat_reset(Connection *con);
void
maybe_update_heartbeat_peers();
void
reset_heartbeat_peers();bool heartbeat_peers_need_update() {
...
public:
bool
heartbeat_dispatch(Message *m);

struct HeartbeatDispatcher: public Dispatcher {
  OSD *osd;
  HeartbeatDispatcher(OSD *o) :
    Dispatcher(cct), osd(o) {
    }
...
check_ops_in_flight();
void
test_ops(std::string command, std::string args, ostream& ss);
...
class ShardedOpWQ: public ShardedThreadPool::ShardedWQ<
		   pair<PGRef, PGQueueable> > {
		     ...
		     public:
		     ShardedOpWQ(uint32_t pnum_shards, OSD *o, time_t ti, time_t si,
			 ShardedThreadPool* tp) :
		       ShardedThreadPool::ShardedWQ<pair<PGRef, PGQueueable> >(ti, si,
			   tp), osd(o), num_shards(pnum_shards) {
		     ...
		     void
		       _process(uint32_t thread_index, heartbeat_handle_d *hb);
		     void
		       _enqueue(pair<PGRef, PGQueueable> item);
		     void
		       _enqueue_front(pair<PGRef, PGQueueable> item);
			 ...
			 for (list<PGQueueable>::reverse_iterator i =
			     iter->second.rbegin(); i != iter->second.rend(); ++i) {
...
void
enqueue_op(PG *pg, OpRequestRef& op);
void
dequeue_op(PGRef pg, OpRequestRef op, ThreadPool::TPHandle &handle);
...
struct PeeringWQ: public ThreadPool::BatchWorkQueue<PG> {
  ...
  PeeringWQ(OSD *o, time_t ti, time_t si, ThreadPool *tp) :
    ThreadPool::BatchWorkQueue<PG>("OSD::PeeringWQ", ti, si, tp), osd(
	o) {
    }
  ...
  void
    _dequeue(list<PG*> *out);
  void _process(const list<PG *> &pgs, ThreadPool::TPHandle &handle) {
    osd->process_peering_events(pgs, handle);
    for (list<PG *>::const_iterator i = pgs.begin(); i != pgs.end();
	++i) {
      (*i)->put("PeeringWQ");
    }
  }
...
void
process_peering_events(const list<PG*> &pg, ThreadPool::TPHandle &handle);
...
void
wait_for_new_map(OpRequestRef op);
void
handle_osd_map(class MOSDMap *m);
void
note_down_osd(int osd);
void
note_up_osd(int osd);

bool
advance_pg(epoch_t advance_to, PG *pg, ThreadPool::TPHandle &handle,
    PG::RecoveryCtx *rctx, set<boost::intrusive_ptr<PG> > *split_pgs);
void
advance_map(ObjectStore::Transaction& t, C_Contexts *tfin);
void
consume_map();
void
activate_map();
...
PGPool
_get_pool(int id, OSDMapRef createmap);

PG *
get_pg_or_queue_for_pg(const spg_t& pgid, OpRequestRef& op);bool
_have_pg(spg_t pgid);
PG *
...
PG *
_open_lock_pg(OSDMapRef createmap, spg_t pg, bool no_lockdep_check =
    false);
enum res_result {
 ...
 */
PG *
_create_lock_pg(OSDMapRef createmap, spg_t pgid,
    bool newly_created,
    bool hold_map_lock,
    bool backfill, int role, vector<int>& up, int up_primary,
    vector<int>& acting, int acting_primary, pg_history_t history,
    pg_interval_map_t& pi, ObjectStore::Transaction& t);
PG *
_lookup_qlock_pg(spg_t pgid);

PG*
_make_pg(OSDMapRef createmap, spg_t pgid);
void
add_newly_split_pg(PG *pg, PG::RecoveryCtx *rctx);

void
handle_pg_peering_evt(spg_t pgid, const pg_info_t& info,
    pg_interval_map_t& pi, epoch_t epoch, pg_shard_t from,
    bool primary, PG::CephPeeringEvtRef evt);
...
void
calc_priors_during(spg_t pgid, epoch_t start, epoch_t end,
    set<pg_shard_t>& pset);

/// project pg history from from to now
bool
project_pg_history(spg_t pgid, pg_history_t& h, epoch_t from,
    const vector<int>& lastup, int lastupprimary,
    const vector<int>& lastacting, int lastactingprimary); ///< @return false if there was a map gap between from and now
...
bool
can_create_pg(spg_t pgid);
void
handle_pg_create(OpRequestRef op);

void
split_pgs(PG *parent, const set<spg_t> &childpgids,
    set<boost::intrusive_ptr<PG> > *out_pgs, OSDMapRef curmap,
    OSDMapRef nextmap, PG::RecoveryCtx *rctx);
 ...
 *  elsewhere.
 */
utime_t last_pg_stats_ack;bool outstanding_pg_stats; // some stat updates haven't been acked yet
...
start_boot();
void
_maybe_boot(epoch_t oldest, epoch_t newest);
void
_send_boot();
void
_collect_metadata(map<string, string> *pmeta);bool
_lsb_release_set(char *buf, const char *str, map<string, string> *pm,
    const char *key);
void
_lsb_release_parse(map<string, string> *pm);

void
start_waiting_for_healthy();bool
_is_healthy();
...
void
queue_want_up_thru(epoch_t want);
void
send_alive();
...
send_failures();
void
send_still_alive(epoch_t epoch, const entity_inst_t &i);

// -- pg stats --
Mutex pg_stat_queue_lock;
Cond pg_stat_queue_cond;
xlist<PG*> pg_stat_queue;bool osd_stat_updated;
uint64_t pg_stat_tid, pg_stat_tid_flushed;

void
send_pg_stats(const utime_t &now);
void
handle_pg_stats_ack(class MPGStatsAck *ack);
void
flush_pg_stats();
...
PG::RecoveryCtx
create_context();bool
compat_must_dispatch_immediately(PG *pg);
void
dispatch_context(PG::RecoveryCtx &ctx, PG *pg, OSDMapRef curmap,
    ThreadPool::TPHandle *handle = NULL);
void
dispatch_context_transaction(PG::RecoveryCtx &ctx, PG *pg,
    ThreadPool::TPHandle *handle = NULL);
void
do_notifies(
    ...
    OSDMapRef map);
void
do_queries(map<int, map<spg_t, pg_query_t> >& query_map, OSDMapRef map);
void
do_infos(map<int, vector<pair<pg_notify_t, pg_interval_map_t> > >& info_map,
    OSDMapRef map);
void
repeer(PG *pg, map<int, map<spg_t, pg_query_t> >& query_map);

bool
require_mon_peer(Message *m);bool
require_osd_peer(Message *m);
/***
 * Verifies that we were alive in the given epoch, and that
 * still are.
 */
bool
require_self_aliveness(Message *m, epoch_t alive_since);
/**
 ...
 */
bool
require_same_peer_instance(Message *m, OSDMapRef& map,
    bool is_fast_dispatch);

bool
require_same_or_newer_map(OpRequestRef& op, epoch_t e,
    bool is_fast_dispatch);

void
handle_pg_query(OpRequestRef op);
void
handle_pg_notify(OpRequestRef op);
void
handle_pg_log(OpRequestRef op);
void
handle_pg_info(OpRequestRef op);
void
handle_pg_trim(OpRequestRef op);

void
handle_pg_backfill_reserve(OpRequestRef op);
void
handle_pg_recovery_reserve(OpRequestRef op);

void
handle_pg_remove(OpRequestRef op);
void
_remove_pg(PG *pg);
...
struct CommandWQ: public ThreadPool::WorkQueue<Command> {
  ...
  CommandWQ(OSD *o, time_t ti, time_t si, ThreadPool *tp) :
    ThreadPool::WorkQueue<Command>("OSD::CommandWQ", ti, si, tp), osd(
	o) {
    }
...
void
handle_command(class MMonCommand *m);
void
handle_command(class MCommand *m);
void
do_command(Connection *con, ceph_tid_t tid, vector<string>& cmd,
    bufferlist& data);
...
void
start_recovery_op(PG *pg, const hobject_t& soid);
void
finish_recovery_op(PG *pg, const hobject_t& soid, bool dequeue);
void
do_recovery(PG *pg, ThreadPool::TPHandle &handle);bool
_recover_now();
...
void
sched_scrub();bool
scrub_random_backoff();bool
scrub_load_below_threshold();bool
scrub_time_permit(utime_t now);

// -- removing --
struct RemoveWQ: public ThreadPool::WorkQueueVal<
		 pair<PGRef, DeletingStateRef> > {
		   ObjectStore *&store;
		   ...
		   void
		     _process(pair<PGRef, DeletingStateRef>, ThreadPool::TPHandle &);
		   void _clear() {
		     remove_queue.clear();
		   }
		 } remove_wq;
...
void
ms_fast_preprocess(Message *m);bool
ms_dispatch(Message *m);bool
ms_get_authorizer(int dest_type, AuthAuthorizer **authorizer,
    bool force_new);bool
ms_verify_authorizer(Connection *con, int peer_type, int protocol,
...
OSD(CephContext *cct_, ObjectStore *store_, int id, Messenger *internal,
    Messenger *external, Messenger *hb_client,
    Messenger *hb_front_server, Messenger *hb_back_server,
    Messenger *osdc_messenger, MonClient *mc, const std::string &dev,
    const std::string &jdev);
~OSD();

// static bits
static int
find_osd_dev(char *result, int whoami);
static int
mkfs(CephContext *cct, ObjectStore *store, const string& dev, uuid_d fsid,
    int whoami);
...
private:
static int
write_meta(ObjectStore *store, uuid_d& cluster_fsid, uuid_d& osd_fsid,
    int whoami);

void
handle_scrub(struct MOSDScrub *m);
void
handle_osd_ping(class MOSDPing *m);
void
handle_op(OpRequestRef& op, OSDMapRef& osdmap);

template<typename T, int MSGTYPE>
void
handle_replica_op(OpRequestRef& op, OSDMapRef& osdmap);

int
init_op_flags(OpRequestRef& op);

public:
static int
peek_meta(ObjectStore *store, string& magic, uuid_d& cluster_fsid,
    uuid_d& osd_fsid, int& whoami);
