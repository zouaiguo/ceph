...
void ACLPermission_Goog::to_xml(ostream& out)
{
...
class ACLID_Goog : public XMLObj
{
public:
  ACLID_Goog() {}
  ~ACLID_Goog() {}
...
class ACLURI_Goog : public XMLObj
{
public:
  ACLURI_Goog() {}
  ~ACLURI_Goog() {}
...
class ACLEmail_Goog : public XMLObj
{
public:
  ACLEmail_Goog() {}
  ~ACLEmail_Goog() {}
...
class ACLDisplayName_Goog : public XMLObj
{
public:
 ACLDisplayName_Goog() {}
 ~ACLDisplayName_Goog() {}
...
bool ACLOwner_Goog::xml_end(const char *el) {
  ACLID_Goog *acl_id = static_cast<ACLID_Goog *>(find_first("ID"));
  ACLID_Goog *acl_name = static_cast<ACLID_Goog *>(find_first("DisplayName"));
...
bool ACLGrant_Goog::xml_end(const char *el) {
  ACLGrantee_Goog *acl_grantee;
  ACLID_Goog *acl_id;
  ACLURI_Goog *acl_uri;
  ACLEmail_Goog *acl_email;
  ACLPermission_Goog *acl_permission;
  ACLDisplayName_Goog *acl_name;
  string uri;

  acl_grantee = static_cast<ACLGrantee_Goog *>(find_first("Grantee"));
  if (!acl_grantee)
    return false;
  string type_str;
  if (!acl_grantee->get_attr("xsi:type", type_str))
    return false;
  ACLGranteeType_Goog::set(type_str.c_str(), type);
  
  acl_permission = static_cast<ACLPermission_Goog *>(find_first("Permission"));
  if (!acl_permission)
    return false;
  ...
  switch (type.get_type()) {
  case ACL_TYPE_CANON_USER:
    acl_id = static_cast<ACLID_Goog *>(acl_grantee->find_first("ID"));
    if (!acl_id)
      return false;
    id = acl_id->to_str();
    acl_name = static_cast<ACLDisplayName_Goog *>(acl_grantee->find_first("DisplayName"));
    if (acl_name)
      name = acl_name->get_data();
    break;
  case ACL_TYPE_GROUP:
    acl_uri = static_cast<ACLURI_Goog *>(acl_grantee->find_first("URI"));
    if (!acl_uri)
      return false;
    ...
    break;
  case ACL_TYPE_EMAIL_USER:
    acl_email = static_cast<ACLEmail_Goog *>(acl_grantee->find_first("EmailAddress"));
    if (!acl_email)
      return false;
...
void ACLGrant_Goog::to_xml(CephContext *cct, ostream& out) {
  ACLPermission_Goog& perm = static_cast<ACLPermission_Goog &>(permission);

  /* only show goog compatible permissions */
  ...
  out << "<Grant>" <<
         "<Grantee xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:type=\"" << ACLGranteeType_Goog::to_string(type) << "\">";
  switch (type.get_type()) {
  case ACL_TYPE_CANON_USER:
    out << "<ID>" << id << "</ID>";
    if (name.size()) {
...
bool ACLGrant_Goog::group_to_uri(ACLGroupTypeEnum group, string& uri)
{
  switch (group) {
...
bool RGWAccessControlList_Goog::xml_end(const char *el) {
  XMLObjIter iter = find("Grant");
  ACLGrant_Goog *grant = static_cast<ACLGrant_Goog *>(iter.get_next());
  while (grant) {
    add_grant(grant);
    grant = static_cast<ACLGrant_Goog *>(iter.get_next());
  }
  return true;
}

struct goog_acl_header {
  int rgw_perm;
  const char *http_header;
};

static const char *get_acl_header(RGWEnv *env,
        const struct goog_acl_header *perm)
{
...
static int parse_grantee_str(RGWRados *store, string& grantee_str,
        const struct goog_acl_header *perm, ACLGrant& grant)
{
...
static int parse_acl_header(RGWRados *store, RGWEnv *env,
         const struct goog_acl_header *perm, std::list<ACLGrant>& _grants)
{
...
int RGWAccessControlList_Goog::create_canned(ACLOwner& owner, ACLOwner& bucket_owner, const string& canned_acl)
{
  acl_user_map.clear();
  grant_map.clear();
...
int RGWAccessControlList_Goog::create_from_grants(std::list<ACLGrant>& grants)
{
  if (grants.empty())
    return -EINVAL;
...
bool RGWAccessControlPolicy_Goog::xml_end(const char *el) {
  RGWAccessControlList_Goog *googacl =
      static_cast<RGWAccessControlList_Goog *>(find_first("AccessControlList"));
  if (!googacl)
    return false;

  acl = *googacl;

  ACLOwner *owner_p = static_cast<ACLOwner_Goog *>(find_first("Owner"));
  if (!owner_p)
    return false;
  owner = *owner_p;
  return true;
...
static const goog_acl_header acl_header_perms[] = {
...
int RGWAccessControlPolicy_Goog::create_from_headers(RGWRados *store, RGWEnv *env, ACLOwner& _owner)
{
  std::list<ACLGrant> grants;

  for (const struct goog_acl_header *p = acl_header_perms; p->rgw_perm; p++) {
 ...
 */
int RGWAccessControlPolicy_Goog::rebuild(RGWRados *store, ACLOwner *owner, RGWAccessControlPolicy& dest)
{
  if (!owner)
    return -EINVAL;

  ACLOwner *requested_owner = static_cast<ACLOwner_Goog *>(find_first("Owner"));
  if (requested_owner) {
  ...
  for (iter = grant_map.begin(); iter != grant_map.end(); ++iter) {
    ...
    case ACL_TYPE_GROUP:
      {
        string uri;
        if (ACLGrant_Goog::group_to_uri(src_grant.get_group(), uri)) {
          new_grant = src_grant;
          grant_ok = true;
...
bool RGWAccessControlPolicy_Goog::compare_group_name(string& id, ACLGroupTypeEnum group)
{
  switch (group) {
  case ACL_GROUP_ALL_USERS:
