		     ...
		     const char *data_dir_option)
{
  ...
  if (getuid() != 0) {
    if (conf->setuser.length()) {
      cerr << "ignoring --setuser " << conf->setuser << " since I am not root"
	   << std::endl;
      conf->set_val("setuser", "", false, false);
    }
    if (conf->setgroup.length()) {
      cerr << "ignoring --setgroup " << conf->setgroup
	   << " since I am not root" << std::endl;
      conf->set_val("setgroup", "", false, false);
    }
  }
  ...
  if (conf->setgroup.length() ||
    ...
    gid_t gid = 0;
    if (conf->setuser.length()) {
      uid = atoi(conf->setuser.c_str());
      if (!uid) {
	char buf[4096];
	struct passwd pa;
	struct passwd *p = 0;
	getpwnam_r(conf->setuser.c_str(), &pa, buf, sizeof(buf), &p);
	if (!p) {
	  cerr << "unable to look up user '" << conf->setuser << "'"
	       << std::endl;
	  exit(1);
	}
	uid = p->pw_uid;
    ...
    if (conf->setgroup.length() > 0) {
      gid = atoi(conf->setgroup.c_str());
      if (!gid) {
	char buf[4096];
	struct group gr;
	struct group *g = 0;
	getgrnam_r(conf->setgroup.c_str(), &gr, buf, sizeof(buf), &g);
	if (!g) {
	  cerr << "unable to look up group '" << conf->setgroup << "'"
	       << std::endl;
	  exit(1);
	}
	gid = g->gr_gid;
    ...
    if ((uid || gid) &&
	conf->setuser_match_path.length()) {
      struct stat st;
      int r = ::stat(conf->setuser_match_path.c_str(), &st);
      if (r < 0) {
	r = -errno;
	cerr << "unable to stat setuser_match_path "
	     << conf->setuser_match_path
	     << ": " << cpp_strerror(r) << std::endl;
	exit(1);
      }
      if ((uid && uid != st.st_uid) ||
	  (gid && gid != st.st_gid)) {
	cerr << "WARNING: will not setuid/gid: " << conf->setuser_match_path
	...
	dout(10) << "setuser_match_path "
		 << conf->setuser_match_path << " owned by "
  ...
  if (conf->run_dir.length() &&
      code_env == CODE_ENVIRONMENT_DAEMON &&
      !(flags & CINIT_FLAG_NO_DAEMON_ACTIONS)) {
    int r = ::mkdir(conf->run_dir.c_str(), 0755);
    if (r < 0 && errno != EEXIST) {
      r = -errno;
      cerr << "warning: unable to create " << conf->run_dir << ": " << cpp_strerror(r) << std::endl;
    }
  }
