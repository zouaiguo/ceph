...
class PrepareDefault(Prepare):

    def __init__(self, args):
        self.data = PrepareData(args)
...
class PrepareData(object):
    ...
    def prepare_file(self, *to_prepare_list):
        ...
        self.populate_data_path(self.args.data, *to_prepare_list)
    ...
    def prepare_device(self, *to_prepare_list):
        ...
        for to_prepare in to_prepare_list:
            to_prepare.prepare()
        self.prepare_dev(*to_prepare_list)
    ...
    def prepare_dev(self, *to_prepare_list):
        if is_partition(self.args.data):
            ...
            LOG.debug('Creating osd partition on %s',
                      self.args.data)
            partition = self.create_data_partition()
